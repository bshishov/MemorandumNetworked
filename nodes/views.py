...
@require_login(url='/login/')
def index(request):
    return text_node(request, request.user.home.id)
...
def text_node(request, id):
    ctx = {'id': id,
           'provider': 'text',
           'node': get_object_or_404(Node, user=request.user.id, id=id)}
    if request.method == 'POST':
...
def url_node(request, id):
    ctx = {'id': id,
           'provider': 'url',
           'node': get_object_or_404(Url, user=request.user.id, url_hash=id)}
    ctx = group_links(get_links(request.user, ctx, id))
    ctx = get_parent_links(request.user, ctx, id)
